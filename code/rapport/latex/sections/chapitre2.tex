...
% CHAPITRE 2
% ==========================================
\chapter{Contexte technique}

\section{Étude de l'existant : Le datamart Construction sous SAS}

\subsection{Rôle du datamart Construction dans l'analyse assurantielle}

Le datamart Construction constitue l'un des datamarts P\&C gérés par l'équipe ADC de la direction P4D d'Allianz France. Il agrège et structure l'ensemble des données relatives au marché de l'assurance construction, qui couvre les risques liés aux bâtiments, travaux et chantiers. Ce marché présente des spécificités métier importantes : les capitaux assurés sont généralement élevés, les sinistres peuvent être complexes et s'étaler sur plusieurs années, et l'indexation des capitaux en fonction de l'inflation dans le secteur de la construction est cruciale.

Le datamart Construction alimente trois types d'analyses principales. Les \textbf{études de portefeuille} permettent de suivre l'évolution du nombre de polices, les mouvements d'affaires nouvelles et de résiliations, et la composition du portefeuille par segment de clientèle (agents, courtiers, entreprises de construction). Les \textbf{analyses de capitaux} portent sur l'évaluation de l'exposition au risque à travers les capitaux assurés (Sinistre Maximum Possible, Limite Contractuelle d'Indemnité, Perte d'Exploitation, Risque Direct) et leur indexation selon les indices de coût de construction de la \hyperlink{ref.ffb_2024}{Fédération Française du Bâtiment (2024} ; \hyperlink{ref.ffb_2025}{2025)}. Enfin, les \textbf{calculs de primes} agrègent les émissions de primes par police et par garantie pour alimenter les reportings financiers et réglementaires.

Les utilisateurs principaux de ce datamart sont les actuaires P4D de l'équipe Midcorp / Construction, ainsi que l'unité Midcorp au sens large grâce à son exploitation via l'outil de Business Intelligence MicroStrategy. La disponibilité et la fiabilité de ce datamart sont critiques : sa fréquence d'actualisation est mensuelle, ce qui impose qu'il soit pleinement opérationnel à chaque début de mois pour garantir la continuité des activités de reporting, de tarification et d'analyse.

La figure \ref{fig:general_sas} ci-dessous illustre la vue d'ensemble du datamart Construction en SAS : les trois sources de données principales (IMS pour le portefeuille AZ et AZEC, OneBI pour les émissions, et les référentiels métier) alimentent trois pipelines distincts produisant trois sorties analytiques finaux.

\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{schemas/general_sas_architecture.png}
    \caption[Vue d'ensemble de l'architecture du datamart Construction]{Vue d'ensemble de l'architecture du datamart Construction : sources de données, transformations et sorties analytiques produits (Voir \protect\hyperlink{annexe.A}{Annexe A})}
    \label{fig:general_sas}
\end{figure}

\subsection{Architecture actuelle en SAS}

L'implémentation actuelle du datamart Construction repose sur une architecture SAS développée progressivement sur plusieurs années. Le code est réparti sur \textbf{dix-neuf fichiers SAS} totalisant environ quinze mille lignes de code, organisés selon une logique séquentielle monolithique.

Comme le montre la figure \ref{fig:sas_detail}, l'architecture détaillée décompose les traitements en trois pipelines indépendants : le \textbf{pipeline Capitaux} (extraction et indexation FFB), le \textbf{pipeline Portefeuille et Mouvements} (nettoyage, fusion AZ/AZEC, enrichissement), et le \textbf{pipeline Émissions} (centralisation et agrégation des primes).

\begin{figure}[H]
    \centering
    \includegraphics[width=1\textwidth]{schemas/sas_detail_flow_architecture.png}
    \caption[Architecture détaillée des flux de données]{Architecture détaillée des flux de données : sources, transformations par étape et tables de sortie des trois pipelines du datamart Construction (Voir \protect\hyperlink{annexe.B}{Annexe B})}
    \label{fig:sas_detail}
\end{figure}

Cette architecture historique présente des défis techniques : utilisation massive de macros imbriquées, variables globales au périmètre ambigu, processus d'exécution strictement séquentiel allongeant les temps de traitement, et une \textbf{absence de documentation initiale} obligeant à une lecture exhaustive du code.
...
\section{Cible technologique : L'écosystème PySpark et l'Architecture Médaillon}

\subsection{PySpark}

\textbf{PySpark} est l'interface Python d'Apache Spark \hyperlink{ref.apache}{(Apache Software Foundation, 2024)}. L'API de DataFrames qu'elle expose permet de manipuler des volumes massifs de données en distribuant les calculs sur un cluster de machines, une capacité absente de l'architecture SAS d'origine. PySpark supporte nativement les opérations SQL, les agrégations complexes, les jointures et les fonctions fenêtres (\textit{window functions}), couvrant ainsi tous les besoins de transformation du datamart Construction.

Pour ce projet, PySpark présente trois avantages décisifs : la compatibilité avec les formats modernes (Parquet, Delta, CSV), la possibilité d'exécuter le même code en local pour les tests ou sur la plateforme ADP, et l'intégration native avec plusieurs plateformes cloud.

\subsection{Allianz Data Platform (ADP)}

\textbf{Allianz Data Platform (ADP)} est l'environnement cloud interne sécurisé fourni par le groupe Allianz pour la gestion et la gouvernance des données au sein de l'entreprise. Basée sur lle cloud Azure et AWS, ADP offre un environnement complet et préconfiguré pour exécuter des \textit{workloads} Spark, avec une intégration native aux stockages de données internes (datalakes) tout en respectant les exigences strictes de sécurité du secteur de l'assurance. Ce choix d'environnement s'aligne avec la stratégie du groupe de standardiser ses infrastructures data, facilitant ainsi la collaboration et le partage de bonnes pratiques.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=1.0\textwidth]{schemas/python_architecture.png}
    \caption[Architecture médaillon implémentée]{Architecture médaillon implémentée : flux de données des sources vers les couches Bronze, Silver et Gold stockées dans Azure Data Lake Storage (ADLS), traitées par Apache Spark et PySpark (Voir \protect\hyperlink{annexe.D}{Annexe D})}
...
Enfin, la \textbf{validation de la parité fonctionnelle} est reconnue comme une étape critique et chronophage. La comparaison des résultats SAS et Python ligne par ligne, indicateur par indicateur, nécessite une méthodologie rigoureuse et un outillage adapté.
...
L'\textbf{adoption d'une architecture modulaire} facilite les tests unitaires et la maintenance. La \textbf{séparation de la configuration et du code} (externalisation des règles métier dans des fichiers JSON ou YAML) rend le code plus lisible et plus facilement évolutif. Enfin, la \textbf{validation incrémentale} (tester chaque module immédiatement après son implémentation) permet de détecter et corriger les erreurs au plus tôt, évitant l'accumulation de bugs difficiles à tracer.
