...
\begin{figure}[H]
    ...
    \caption{Vue d'ensemble de l'architecture du datamart Construction : sources de données, transformations et sorties analytiques produits}
...
\begin{figure}[H]
    ...
    \caption{Architecture détaillée des flux de données : sources, transformations par étape et tables de sortie des trois pipelines du datamart Construction}
...
\textbf{Azure Databricks} est une plateforme cloud managée qui fournit un environnement complet pour exécuter des workloads Spark : clusters gérés automatiquement, notebooks collaboratifs, intégration native avec Azure Data Lake Storage (ADLS) et support de Delta Lake (Microsoft Corporation, 2024). Le choix d'Azure Databricks s'aligne avec la stratégie du groupe Allianz, qui a standardisé ses infrastructures data sur Azure au niveau international. Cette convergence facilite le partage de bonnes pratiques entre les entités du groupe.
...
\begin{figure}[H]
    ...
    \caption{Architecture médaillon implémentée : flux de données des sources vers les couches Bronze, Silver et Gold stockées dans Azure Data Lake Storage (ADLS), traitées par Apache Spark et PySpark}
