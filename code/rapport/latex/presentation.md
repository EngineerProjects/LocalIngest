...
Mon travail a porté sur un enjeu stratégique majeur pour l'équipe (Allianz France) qui est la migration du Datamart Construction, depuis SAS vers une architecture moderne sous Python et PySpark.
...
Le constat de départ était simple : l'architecture historique sous SAS n'était plus totalement en phase avec nos besoins. C'est une technologie sous Licence qui n'est donc pas ouverte aux changements facilement, il est rigide, et peine face à l'augmentation des volumes de données. De plus, le code était devenu très complexe à maintenir.

Face à cela, l'objectif était clair : basculer vers un environnement moderne, PySpark. Cela nous apporte la puissance de l'Open Source, et surtout, cela nous aligne parfaitement avec la stratégie Data du groupe Allianz (Databricks). Bref : plus de performance, et une réduction des coûts.
...
Une fois cette phase d'analyse terminée, nous avons pu définir notre architecture cible : l'Architecture Médaillon, qui est un standard reconnu sur le marché.
Pourquoi ce choix précis ?
Pour trois raisons fondamentales :
D'abord, la **Traçabilité** : grâce à cette séparation en 3 couches (Bronze, Silver, Gold), on conserve toujours les données brutes. En cas d'erreur de calcul, on peut remonter à la source sans rien perdre.
Ensuite, la **Performance** : on utilise le format Parquet, qui est colonnaire et infiniment plus rapide à requêter que les anciens fichiers CSV ou SAS.
Et enfin, c'est le **Standard** d'architecture qui nous permet d'exploiter toute la puissance du "Compute" avec Apache Spark et Python, de manière totalement intégrée sur le Datalake Azure d'Allianz.
