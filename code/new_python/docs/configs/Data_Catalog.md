...
|-------|-------|--------------|
| **Bronze (Inputs)** | 36 file groups | `bronze/{YYYY}/{MM}/` or `bronze/ref/` |
| **Silver (Intermediate)** | 4 tables | `silver/{YYYY}/{MM}/` |
...
| # | Table | Description |
|---|-------|-------------|
| 5 | `primes_emises_{vision}_pol_garp.parquet` | Premiums by guarantee |
| 6 | `primes_emises_{vision}_pol.parquet` | Premiums by policy |
...
|---|-------|--------|-------------|
| 1 | `mvt_const_ptf_{vision}` | AZ Processor | → Gold consolidation |
| 2 | `azec_ptf_{vision}` | AZEC Processor | → Gold consolidation |
| 3 | `az_capitaux_{vision}` | AZ Capitaux | → Gold consolidation |
| 4 | `azec_capitaux_{vision}` | AZEC Capitaux | → Gold consolidation |
...
**Vision Format:** YYYYMM (e.g., 202509)
