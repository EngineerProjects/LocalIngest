...
\begin{itemize}
    \item \textbf{L'accès aux données de test :} Les validations de sécurité pour accéder aux données Azure ont souvent imposé de coder et tester avec des données fictives en local (mocks), allongeant ainsi le temps de développement.
    \item \textbf{Le pôle pour AZEC :} En Python, le décompte des polices pour le Courtage (Pôle 3) était inférieur à celui de SAS. L'analyse du code source a révélé que SAS forçait manuellement la valeur \texttt{cdpole = "3"} via une macro dédiée. Cette logique a été reproduite en Python pour rétablir l'équilibre.
    \item \textbf{Les formats de dates hétérogènes :} Certaines colonnes de dates (\texttt{dtechann}) présentaient des formats historiques mélangés (MMJJ, MMJJAA). Un format Date natif dans Spark provoquant des erreurs, ces colonnes ont été traitées comme du texte (similairement à SAS) pour convertir toutes les lignes sans incident.
    \item \textbf{Les dates SAS (\texttt{datetime}) :} Les colonnes issues d'IRD Risk au format \texttt{'14MAY1991:00:00:00'} n'étaient pas reconnues nativement par PySpark, produisant des valeurs nulles. Une fonction utilitaire (\texttt{parse\_sas\_datetime()}) a été développée pour traiter ce cas spécifique.
    \item \textbf{L'utilisation des configurations JSON :} L'utilisation directe de filtres JSON avec une fonction \texttt{isin()} dans PySpark provoquait des erreurs d'interprétation. Il a fallu extraire explicitement les valeurs associées sous forme de liste en Python pour assurer la compatibilité avec Spark.
\end{itemize}
