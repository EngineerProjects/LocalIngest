...
Le projet s'est déroulé au sein d'une équipe restreinte, composée de l'étudiant et de son chef de projet, Romeo TOHOUN, Data Scientist, et de sa responsable hiérarchique Caroline BOUCHON, dans l'équipe Advanced Data \& Climate. Des points d'avancement hebdomadaires ont permis de valider les choix techniques, de lever les ambiguïtés métier et d'orienter les priorités de développement. La collaboration avec M. Alain VOSS, expert assurance de l'équipe, a été essentielle pour comprendre les mécanismes assurantiels sous-jacents aux règles métier codées en SAS.
...
Suite à l'analyse du code SAS, deux options d'architecture ont été évaluées :

\begin{itemize}
    \item \textbf{La réplication à l'identique} (un script Python pour un fichier SAS) : plus simple pour valider les résultats rapidement, mais conserve tous les défauts de l'ancien code SAS (séquentialité, trop de dépendances). Option écartée.
    \item \textbf{L'architecture médaillon complète} (Bronze/Silver/Gold) : plus longue à mettre en place initialement, mais garantit la qualité, une bonne traçabilité et une grande évolutivité. C'est l'option retenue.
\end{itemize}
...
Les \textbf{fondations du projet} (module de lecture, helpers, logger, configuration) ont été développées et testées en premier, avant le développement des pipelines métier. Cela a permis d'établir une base technique solide et fiable pour la suite.

\subsection{Ordre d'implémentation et stratégie de développement}

L'implémentation a commencé par le pipeline le plus complexe : \textit{PTF Mouvements} (consolidation AZ/AZEC et nombreux enrichissements), puis \textit{Capitaux} (pour gérer l'indexation FFB), et enfin \textit{Émissions} (plus linéaire).

Débuter par le périmètre le plus difficile a mené à créer très tôt des fonctions utilitaires (calculs AFN/RES, enrichissement par référentiels) directement réutilisables pour les autres pipelines, générant un gain de temps significatif.

Pour chaque pipeline, une méthode itérative a été adoptée : coder une transformation, la tester sur un échantillon de données, vérifier les logs, puis passer à la suite. Cette boucle courte permet de repérer et corriger les erreurs immédiatement.
...
Lors de la comparaison des résultats entre Python et SAS, plusieurs défis ont d\^u être relevés pour atteindre la parité parfaite :

\begin{itemize}[label={---}]
    \item \textbf{Disponibilité des données de test :} L'accès aux données de production sur Azure nécessitait des validations de sécurité strictes. Ces délais ont souvent imposé de coder « à l'aveugle » en attendant de pouvoir tester sur des données réelles, rallongeant ainsi les cycles de correction.
    \item \textbf{Colonne \texttt{cdpole} absente en AZEC :} Lors de la consolidation, Python trouvait moins de polices Courtage (pôle 3) que SAS. L'analyse a montré que SAS forçait l'assignation \texttt{cdpole = "3"} via une macro dédiée. Un simple ajout dans le processor AZEC a rétabli la parité.
    \item \textbf{Formats de dates hétérogènes (\texttt{dtechann}) :} La date d'échéance annuelle mélangeait plusieurs formats historiques (\texttt{MMJJ}, \texttt{MMJJAA}, formats complets). Plutôt que d'utiliser un type Date natif provoquant des erreurs sur les anomalies, la logique SAS a été reproduite en traitant la colonne comme du texte pour extraire et recombiner les caractères, couvrant ainsi 100\,\% des cas.
    \item \textbf{Dates au format SAS \texttt{datetime} (IRD) :} Les tables IRD Risk contenaient des chaînes complexes (ex~: \texttt{'14MAY1991:00:00:00'}) non reconnues par PySpark, produisant des valeurs nulles silencieuses. Un helper \texttt{parse\_sas\_datetime()} a été développé pour extraire et convertir spécifiquement la partie date.
    \item \textbf{Configurations JSON capricieuses :} Pour le filtrage des Émissions métier, le JSON avait été structuré avec descriptions et valeurs pour en faciliter la lecture. Cependant, utiliser ce dictionnaire directement dans un \texttt{isin()} PySpark déclenchait une erreur. Il a fallu extraire explicitement la liste des valeurs en Python pour assurer la compatibilité avec Spark.
    \item \textbf{Harmonisation AZ vs AZEC :} Les deux canaux présentaient des noms de colonnes différents (ex~: \texttt{POLICE} vs \texttt{NOPOL}). Pour éviter un code rigide rempli de \texttt{withColumnRenamed}, l'ensemble du mapping a été externalisé dans un JSON dédié, permettant de renommer les colonnes dynamiquement avant l'union.
\end{itemize}
...
Le développement a été réalisé dans un environnement cloud ADP (Allianz Data Platform) qui est automatiquement connecté à Azure via un token, permettant de tester l'intégralité des pipelines sans nécessiter de connexion complexe à l'infrastructure cloud Azure. Cette autonomie a facilité le cycle de développement itératif, réduisant les temps de feedback entre la modification et le test. La migration vers Azure Databricks est prévue pour la phase de déploiement en production, postérieure à la période du projet.
...
\noindent\fbox{
    \begin{minipage}{\textwidth}
        \vspace{0.2cm}
        \begin{center}
            {\large \textbf{Synthèse des livrables produits lors du projet}}
        \end{center}
        \vspace{0.4em}
