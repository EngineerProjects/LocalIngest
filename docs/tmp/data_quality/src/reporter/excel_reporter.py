...
def _write_summary_sheet(ws, stats: dict, header_font, header_fill, title_font) -> None:
    ...
    for label, value in [
        ...
        ("Sites sans anomalie", format_number(stats["sites_ok"])),
        ("Sites avec anomalie(s)", format_number(stats["sites_avec_anomalies"])),
        ("Sites validés par GPS", format_number(stats["gps_ok"])),
        ("Sites validés par adresse", format_number(stats["adresse_ok"])),
        ("Sites incomplets", format_number(stats["sites_incomplets"])),
    ]:
